...
extension Stopwatch
{
    ...
    static var reducer: Reducer<Action, State, Environment>
    {
        .init { action, state, environment in
            ...
            case (.start, .idle):
                ...
                let getStartDate = DateUtil.getDate(next: Action._didStart)
                return getStartDate(environment.getDate)
                    .toEffect(id: GetStartDateID())
    ...
    struct GetStartDateID: EffectIDProtocol {}
...
extension Stopwatch
{
    private static func timerEffect(startDate: Date) -> Effect<Action>
    {
        Timer.publish(every: 0.01, tolerance: 0.01, on: .main, in: .common)
            .autoconnect()
            .map { Action._update(start: startDate, current: $0) }
            .toEffect(id: TimerID())
    }
}
