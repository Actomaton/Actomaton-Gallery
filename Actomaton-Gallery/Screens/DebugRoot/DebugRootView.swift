...
{
    private let store: Store<DebugRoot.Action, DebugRoot.State>.Proxy
    private let usesTimeTravel: Bool

    init(store: Store<DebugRoot.Action, DebugRoot.State>.Proxy, usesTimeTravel: Bool = true)
    {
        self.store = store
    ...
    var body: some View
    {
        let rootView = RootView(
            store: self.store.timeTravel.inner
                .contramap(action: { DebugRoot.Action.timeTravel(.inner($0)) })
        )
            ...
            Slider(
                value: self.store.timeTravel.timeTravellingSliderValue
                    .stateBinding(onChange: {
                        DebugRoot.Action.timeTravel(.timeTravelSlider(sliderValue: $0))
                    }),
